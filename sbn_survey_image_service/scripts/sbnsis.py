...
def status_service(args: argparse.Namespace) -> int:
    ...
    processes: List[str] = subprocess.check_output(
        ["ps", "-ef"]).decode().splitlines()
    venv: str = os.getenv("VIRTUAL_ENV")
    gunicorn_processes: List[str] = [process for process in processes
                                     if f"{venv}/bin/sbnsis-service" in process]

    n: int = len(gunicorn_processes)
    if n == 0:
        print("No sbnsis-service is running from this project's virtual environment at the moment.")
        return 0
    ...
    print(f"""sbnsis-service is running with {n} processes.
Parent PID: {ppid}""")
