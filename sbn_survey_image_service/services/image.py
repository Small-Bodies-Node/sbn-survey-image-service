    ...
    def cutout(self, url: str) -> str:
        ...
        with fits.open(url, **options) as data:
            ...
            cutout: Cutout2D = Cutout2D(
                data[i].section, self.coords, self.size, wcs=wcs
            )

            header.update(cutout.wcs.to_header())

            result.append(fits.PrimaryHDU(cutout.data, header))
            result.writeto(fits_image_path)
...
def image_query(
    ...
    fits_image_path: str = cutout_spec.cutout(im.image_url)
